...
const Home = () => {
  ...
  <h2 className="text-white font-medium">{time}</h2>
        ...
        <div className='flex flex-col mt-11 gap-10 items-start jusity-start text-white/50 '>
          ...
          <FaFolder onClick={()=>setShowProject(true)} className='h-13 w-20 ml-11 -mt-28 hover:text-white/70'/>
          <p className='text-white ml-13 -mt-2'>Projects</p>
          <div className='w-22 h-22 bg-white/40 rounded-xl ml-10'></div>
          <FaPhone onClick={()=>setShowContact(true)} className='h-13 w-20 ml-11 -mt-28  hover:text-white/70 '/>
          <p className='text-white ml-13 -mt-2'>Contact</p>
           <div className='w-22 h-22 bg-white/40 rounded-xl ml-10'></div>
          <FaUser onClick={()=>setShowAbout(true)} className='h-12 w-20 ml-11 hover:text-white -mt-27'/>
...
<p className="capitalize">{weather?.weather?.[0]?.description}</p>
      ...
      {showProject && 
        <Projects onClose={()=>setShowProject(false)} />}
        {showContact&& 
